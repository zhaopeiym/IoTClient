...
namespace IoTClient.Clients.PLC
{
    ...
    public class SiemensClient : SocketBase
    {
        ...
        /// 版本
        /// </summary>
        public readonly SiemensVersion version;
        /// <summary>
        /// IPEndPoint
        /// </summary>
        public readonly IPEndPoint ipAndPoint;
        /// <summary>
        /// 超时时间
        /// </summary>
        ...
        public Result<byte[]> SendPackage(byte[] command)
        {
            ...
            lock (this)
            {
                ...
                var headPackage = SocketTryRead(socket, SiemensConstant.InitHeadLength);
                //如果出现异常，则进行一次重试
                if (headPackage == null)
                {
                    //重新打开连接
        ...
        /// <returns></returns>
        public Result<Dictionary<string, object>> Read(Dictionary<string, DataTypeEnum> addresses, int batchNumber = 19)
        {
        ...
        private Result<Dictionary<string, object>> BatchRead(Dictionary<string, DataTypeEnum> addresses)
        {
            ...
            try
            {
                ...
                var length = args.Sum(t => t.ReadLength == 1 ? 2 : t.ReadLength) + args.Length * 4;
                if (args.Last().ReadLength == 1) length--;//TODO 最后一个如果是Bool，结果会少一个字节？
                byte[] responseData = new byte[length];
                ...
                foreach (var item in args)
                {
                    object value;
                    cursor += 4;
                    var readResut = responseData.Skip(cursor).Take(item.ReadLength).Reverse().ToArray();
                    cursor += item.ReadLength == 1 ? 2 : item.ReadLength;
        ...
        protected byte[] GetReadCommand(SiemensData[] datas)
        {
            ...
            for (int i = 0; i < datas.Length; i++)
            {
                ...
                command[22 + i * 12] = data.ReadBit ? (byte)0x01 : (byte)0x02;//Transport size: BYTE 
                command[23 + i * 12] = (byte)(data.ReadLength / 256);
                command[24 + i * 12] = (byte)(data.ReadLength % 256);//[23][24]两个字节,访问数据的个数，以byte为单位；
        ...
        protected byte[] GetReadBitCommand(byte type, int beginAddress, ushort dbAddress, ushort length = 1)
        {
            ...
            command[20] = 0x0A;
            command[21] = 0x10;
            command[22] = 0x01; //此处和 GetReadCommand 的不一样
